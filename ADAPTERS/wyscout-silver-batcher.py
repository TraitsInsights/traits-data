...
args = getResolvedOptions(sys.argv, ['JOB_NAME', 'seasons_list', 'priority_group', 'min_year', 'max_year', 'basic'])
try:
...
if seasons_list:
    seasons_lol = batch_list(seasons_list, 5)
else:
    for competitionId in comps_ls:
        ...
        if _seasons:
            ...
            valid_seasons = seasons_df[(seasons_df['startYear'] >= min_year) & (seasons_df['startYear'] <= max_year) & (pd.to_datetime(seasons_df['season.startDate']) < datetime.now())]
...
for seasons_batch in seasons_lol:
    if seasons_batch:
        response = glue_client.start_job_run(
            JobName='SILVER-adapter-wyscout', #wyscout-silver-batch',
            Arguments={
                #'--competition_id': str(competitionId),
                '--seasons_to_update': json.dumps(seasons_batch)
            }
        )
