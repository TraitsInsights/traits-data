...
for season in seasons_to_process:
    ...
    # Write three times for different aggregation levels

    for agg in ['season', 'last_four', 'last_eight']:
    
        if agg == 'season':
            table_suffix = 'Output'
        elif agg == 'last_four':
            table_suffix = 'LastFour'
        elif agg == 'last_eight':
            table_suffix = 'LastEight'
        ...
        write_df = final.copy()
        write_df = write_df.loc[write_df['aggregationPeriod']==agg]
        ...
        table_name = f"id_{client_id}.{table_suffix}"
    
        # Create table if not existing
        
        alter_statements = [
            f"CREATE SCHEMA IF NOT EXISTS id_{client_id}",
            gen_tbl_cols_sql(final, f'id_{client_id}.{table_suffix}'),
            f"ALTER TABLE id_{client_id}.{table_suffix} CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci",
            f"""ALTER TABLE id_{client_id}.{table_suffix}
            ...
            f"CREATE FULLTEXT INDEX playerUniversalFullTextIndex ON id_{client_id}.{table_suffix}(fullNameNormalised, playerNameNormalised, teamName, competitionName);"
            ]
        ...
        if len(response['records']) >= 1:
            ...
            response = execute_statement(f'SHOW TABLES IN id_{client_id} LIKE "{table_suffix}"')
            if len(response['records']) >= 1:
                print("Table id_{}.{} exists.".format(client_id, table_suffix))
                mysql_schema = execute_statement("DESCRIBE id_{}.{}".format(client_id, table_suffix))['records']
                ...
                print("Table id_{}.{} created.".format(client_id,table_suffix))
                mysql_schema = execute_statement("DESCRIBE id_{}.{}".format(client_id,table_suffix))['records']
                mysql_columns = [row[0]['stringValue'] for row in mysql_schema][1:]
                mysql_dtypes = [row[1]['stringValue'] for row in mysql_schema][1:]
        else:
        ...
        # print("DTYPES... CHECKED")
         ...
         ### OPTIONAL FOR EDA: write to csv
        # write_df.to_csv(f's3://traits-app-wyscout/silver-cleaned/{season}.csv/')
...
#     AS SELECT DISTINCT competitionName AS name FROM id_{client_id}.{table_suffix};""",
#     f"""CREATE TABLE IF NOT EXISTS id_{client_id}.season
#     AS SELECT DISTINCT seasonName AS name FROM id_{client_id}.{table_suffix};""",
#     ]
